...
current_balance = rpc.getbalance()
...
address_A = rpc.getnewaddress("Address_A", "p2sh-segwit")
address_B = rpc.getnewaddress("Address_B", "p2sh-segwit")
address_C = rpc.getnewaddress("Address_C", "p2sh-segwit")
...
print("\nSent 6 BTC to Address A'. Funding txid:", txid_fund_A)
...
for vout in decoded_tx_A_B["vout"]:
    if "addresses" in vout["scriptPubKey"]:
        if address_B in vout["scriptPubKey"]["addresses"]:
            scriptPubKey_B = vout["scriptPubKey"]["hex"]
            break
    elif "address" in vout["scriptPubKey"]:
        if address_B == vout["scriptPubKey"]["address"]:
            scriptPubKey_B = vout["scriptPubKey"]["hex"]
            break
...
print("Change amount for B'->C':", change_amount_B)
...
print("Decoded ScriptSig from transaction B'->C' (response script):", scriptSig_B)
print("\n--- Report Summary ---")
print("Workflow:")
print("1. Transaction from A' to B' (txid):", txid_A_B)
print("   - This transaction funded Address B' and produced a UTXO with the following locking script (challenge script):")
print("     ", scriptPubKey_B)
print("2. Transaction from B' to C' (txid):", txid_B_C)
print("   - The UTXO from transaction A'->B' (txid:", input_txid_B, ") was used as input for this transaction.")
print("\nDecoded Scripts:")
print("  - Challenge Script (ScriptPubKey for Address B'):", scriptPubKey_B)
print("  - Response Script (ScriptSig in the input spending from Address B'):", scriptSig_B)
print("\nExplanation:")
print("  The challenge script (locking script) defines the conditions (using a P2SH-SegWit format) that must be met to spend the output.")
print("  The response script (unlocking script) provides the signature and public key that, when evaluated against the challenge script,")
print("  validate the spending of the UTXO. You can analyze these scripts using a Bitcoin debugger to step through the evaluation")
print("  process and ensure that the conditions are correctly met.")
